  ...
  <div class="page">
    <div class="home-header">
      ...
      <div class="month-scroll">
        <button v-for="m in state.months" :key="m.id" class="month-chip"
          :class="{ active: state.currentMonthId === m.id }" @click="selectMonth(m.id)">
          {{ m.label.split(' ')[0] }}
        </button>
      </div>
    </div>
          ...
          <div class="savings-bar-wrap">
            <div class="savings-bar">
              <div class="savings-fill"
                :style="{ width: spendPercent + '%', background: spendPercent > 100 ? 'var(--red)' : 'var(--accent)' }">
              </div>
            </div>
...
// Fix: mostra solo la parte locale della email (prima della @)
const userName = computed(() => {
  const email = state.user?.email || ''
  return email.split('@')[0] || 'tu'
})
...
// Carica al mount iniziale
onMounted(async () => {
  if (!state.months.length) await loadMonths()
  if (state.currentMonthId) await caricaMese(state.currentMonthId)
})

// Fix Android PWA: ricarica quando si torna sulla Home (es. da un'altra tab)
// onActivated funziona con <keep-alive>, ma usiamo anche un watcher sul visibilitychange
if (typeof document !== 'undefined') {
  document.addEventListener('visibilitychange', async () => {
    if (document.visibilityState === 'visible' && state.user && state.currentMonthId) {
      // Ricarica silenziosamente senza mostrare skeleton
      await loadTransactions(state.currentMonthId)
    }
  })
}

// Watch sul mese corrente: se cambia (es. dopo aggiunta transazione) ricarica
watch(() => state.currentMonthId, async (newId) => {
  if (newId && !currentTransactions.value.length) {
    await caricaMese(newId)
  }
})
</script>
