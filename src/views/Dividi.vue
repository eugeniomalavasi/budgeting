        ...
        <div class="sheet">
          ...
          <div class="edit-split-section">
            ...
            <div class="split-modes">
              <button :class="['split-mode-btn', editSplitType === 'equal' && 'active']" @click="setEditSplit('equal')">
                <span class="sm-icon">⚖️</span><span class="sm-label">50/50</span>
              </button>
              <button :class="['split-mode-btn', editSplitType === 'mine' && 'active']" @click="setEditSplit('mine')">
                <span class="sm-icon">👤</span><span class="sm-label">Tutto a me</span>
              </button>
              <button :class="['split-mode-btn', editSplitType === 'theirs' && 'active']" @click="setEditSplit('theirs')">
                <span class="sm-icon">👥</span><span class="sm-label">Tutto a {{ nomeAltro }}</span>
              </button>
            </div>
            <div class="split-preview">
              <div class="split-person">
                <span class="sp-name">{{ state.profile?.name }}</span>
                <span class="sp-amount neg amount">{{ fmtFull(editShareIo) }}</span>
              </div>
              <div class="split-person">
                <span class="sp-name">{{ nomeAltro }}</span>
                <span class="sp-amount neg amount">{{ fmtFull(editShareAltro) }}</span>
              </div>
            </div>
          </div>
...
const editSplitType = ref('equal')
const editShareIo = ref(0)
const editShareAltro = ref(0)
...
const isEu = computed(() => state.profile?.name === 'Eugenio')

function quotaMia(s) {
  return isEu.value ? Number(s.share_eu) : Number(s.share_ma)
}
function quotaAltro(s) {
  return isEu.value ? Number(s.share_ma) : Number(s.share_eu)
}

function splitLabel(s) {
  if (s.split_type === 'equal') return '50/50'
  if (s.split_type === 'mine') return 'Tutto a ' + (state.profile?.name || 'te')
  if (s.split_type === 'theirs') return 'Tutto a ' + nomeAltro.value
...
function apriSheet(s) {
  selected.value = s
  // Precarica i valori di modifica dal record esistente
  editSplitType.value = s.split_type || 'equal'
  editShareIo.value = quotaMia(s)
  editShareAltro.value = quotaAltro(s)
}

function setEditSplit(mode) {
  editSplitType.value = mode
  const tot = selected.value?.importo_totale || 0
  if (mode === 'equal') {
    editShareIo.value = Math.round(tot / 2 * 100) / 100
    editShareAltro.value = Math.round((tot - editShareIo.value) * 100) / 100
  } else if (mode === 'mine') {
    editShareIo.value = tot; editShareAltro.value = 0
  } else {
    editShareIo.value = 0; editShareAltro.value = tot
  }
}
...
async function salvaSuddivisione() {
  ...
  try {
    await updateSharedExpense(selected.value.id, {
      split_type: editSplitType.value,
      share_eu: isEu.value ? editShareIo.value : editShareAltro.value,
      share_ma: isEu.value ? editShareAltro.value : editShareIo.value,
      importo_totale: selected.value.importo_totale,
    })
    // Aggiorna la selected per riflettere i nuovi valori subito
    selected.value = {
      ...selected.value,
      split_type: editSplitType.value,
      share_eu: isEu.value ? editShareIo.value : editShareAltro.value,
