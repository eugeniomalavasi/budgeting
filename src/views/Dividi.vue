  ...
  <div class="page">
    ...
    <div class="px">
      ...
      <div v-if="!unsettled.length && !settled.length" class="empty-state">
        <span>🤝</span><p>Nessuna spesa condivisa</p>
      ...
      <div v-if="selected" class="sheet-overlay" @click.self="selected=null">
        <div class="sheet">
          ...
          <div class="sheet-details">
            ...
            <div class="sheet-row"><span>Quota {{ state.profile?.name }}</span><span class="neg amount">{{ fmtFull(quotaMia(selected)) }}</span></div>
          ...
          <div class="edit-split-section">
            ...
            <div class="split-options">
              ...
              <button :class="['split-opt', editSplitMode==='ma_meta'&&'active']" @click="setEditSplitMode('ma_meta')">
                <div class="so-top"><span class="so-payer">{{ nomeAltro }} paga</span><span class="so-badge">50/50</span></div>
                <div class="so-amounts" v-if="selected?.importo_totale">
                  <span>Eu {{ fmtFull(-selected.importo_totale/2) }}</span>
                  <span>{{ nomeAltro }} {{ fmtFull(-selected.importo_totale/2) }}</span>
                </div>
              </button>
              <button :class="['split-opt', editSplitMode==='eu_tutto'&&'active']" @click="setEditSplitMode('eu_tutto')">
                <div class="so-top"><span class="so-payer">Eugenio paga</span><span class="so-badge so-badge-red">{{ nomeAltro }} deve tutto</span></div>
                <div class="so-amounts" v-if="selected?.importo_totale">
                  <span class="pos">Eu +{{ fmtFull(selected.importo_totale) }}</span>
                  <span class="neg">{{ nomeAltro }} {{ fmtFull(-selected.importo_totale) }}</span>
                </div>
              </button>
              <button :class="['split-opt', editSplitMode==='ma_tutto'&&'active']" @click="setEditSplitMode('ma_tutto')">
                <div class="so-top"><span class="so-payer">{{ nomeAltro }} paga</span><span class="so-badge so-badge-red">Eugenio deve tutto</span></div>
                <div class="so-amounts" v-if="selected?.importo_totale">
...
import { state, saldoCondiviso, loadSharedExpenses, settleExpense, settleAll,
         updateSharedExpense, fmtFull } from '../lib/store.js'
...
.card-green { border-color:rgba(48,209,88,0.3); }
.card-red   { border-color:rgba(255,95,87,0.3); }
...
.settle-btn { background:linear-gradient(135deg,var(--accent),var(--accent2)); border:none; border-radius:12px; color:#0e0e0e; font-family:'Lexend',sans-serif; font-size:0.9rem; font-weight:700; padding:0.65rem 1.25rem; cursor:pointer; }
...
.sr-split { font-size:0.72rem; color:var(--accent); font-weight:600; margin-top:2px; }
...
.sheet { background:var(--surface); border-radius:24px 24px 0 0; border-top:1px solid var(--border); padding:1rem 1.5rem 2rem; width:100%; max-width:480px; display:flex; flex-direction:column; align-items:center; gap:0.75rem; max-height:90dvh; overflow-y:auto; }
.sheet-handle { width:36px; height:4px; background:var(--border); border-radius:100px; margin-bottom:0.25rem; flex-shrink:0; }
.sheet-desc { font-size:1.1rem; font-weight:600; }
.sheet-amount { font-size:1.8rem; font-weight:700; font-family:'DM Mono',monospace; }
...
.edit-split-section { width:100%; display:flex; flex-direction:column; gap:0.65rem; }
.edit-split-title { font-size:0.72rem; font-weight:600; color:var(--text2); text-transform:uppercase; letter-spacing:0.08em; }
.split-options { display:flex; flex-direction:column; gap:0.4rem; width:100%; }
.split-opt { background:var(--surface2); border:1px solid var(--border); border-radius:12px; color:var(--text); cursor:pointer; font-family:'Lexend',sans-serif; padding:0.7rem 0.9rem; text-align:left; transition:all 0.2s; display:flex; flex-direction:column; gap:3px; }
.split-opt.active { background:rgba(245,166,35,0.1); border-color:var(--accent); }
.so-top { display:flex; align-items:center; gap:0.5rem; }
.so-payer { font-size:0.88rem; font-weight:600; }
.so-badge { font-size:0.68rem; font-weight:600; padding:2px 7px; border-radius:100px; background:rgba(245,166,35,0.2); color:var(--accent); }
.so-badge-red { background:rgba(255,95,87,0.15); color:var(--red); }
.so-amounts { display:flex; gap:1rem; font-size:0.75rem; font-family:'DM Mono',monospace; }
.pos { color:var(--green); }
.neg { color:var(--red); }
.save-split-btn { width:100%; background:linear-gradient(135deg,var(--accent),var(--accent2)); border:none; border-radius:14px; color:#0e0e0e; font-family:'Lexend',sans-serif; font-size:0.95rem; font-weight:700; padding:0.85rem; cursor:pointer; }
.save-split-btn:disabled { opacity:0.6; }
.settle-single-btn { width:100%; background:rgba(48,209,88,0.12); border:1px solid rgba(48,209,88,0.3); border-radius:14px; color:var(--green); font-family:'Lexend',sans-serif; font-size:0.95rem; font-weight:600; padding:0.8rem; cursor:pointer; }
