    ...
    <div class="px">
      ...
      <template v-else>
        ...
        <div class="tx-list card" v-if="transazioniFiltrate.length">
          <div
            v-for="tx in transazioniFiltrate"
            :key="tx.id"
            class="tx-row"
            @click="selected = tx"
          >
    ...
    <transition name="sheet">
      <div v-if="selected" class="sheet-overlay" @click.self="selected = null">
        <div class="sheet">
          ...
          <p class="sheet-amount amount" :class="selected.importo < 0 ? 'neg' : 'pos'">
            {{ fmtFull(selected.importo) }}
          </p>
          ...
          <button
            v-if="selected.created_by === state.user?.id"
            class="delete-btn"
            @click="elimina(selected)"
          >🗑 Elimina</button>
...
.empty-state {
  ...
  gap: 0.5rem; padding: 3rem; color: var(--text2);
}
